...
const gridSize = parseInt(rootStyles.getPropertyValue('--grid-size'));
const cellSize = parseInt(rootStyles.getPropertyValue('--cell-size'));

let drawingData = Array.from({ length: gridSize }, () => Array(gridSize).fill('#FFFFFF'));
...
function drawCell(row, col) {
    ...
    const cell = drawingArea.children[row * gridSize + col];
    cell.style.backgroundColor = color;
}

function clearDrawingArea() {
    drawingData = Array.from({ length: gridSize }, () => Array(gridSize).fill('#FFFFFF'));
